  ...
  return parseManifest({
    ...
    tools: [
      ...
      },
      {
        ...
        description: "Aulas com vaga disponível na academia, com horário, professor e ocupação.",
        path: "/api/v1/activities/schedule",
        scope: "business",
        params: [
          { name: "date", in: "query", description: "Dia da grade no formato AAAA-MM-DD. Vazio = hoje." }
        ],
        query: { onlyAvailables: "true", take: "20", ...branch },
        root: ["$"],
        fields: [
          { path: "[].name", label: "Aula" },
          { path: "[].activityDate", label: "Data" },
          { path: "[].startTime", label: "Início" },
          { path: "[].instructor", label: "Professor" },
          { path: "[].capacity", label: "Vagas" },
          { path: "[].ocupation", label: "Ocupadas" }
        ],
        maxChars: 1500
      },
      {
        name: "evo_modalidades",
        description: "Modalidades e atividades oferecidas pela academia.",
        path: "/api/v1/activities",
        scope: "business",
        params: [
          { name: "search", in: "query", description: "Filtra por nome da modalidade, quando o cliente citar uma." }
        ],
        query: { take: "30", ...branch },
        root: ["$"],
        fields: [
          { path: "[].name", label: "Modalidade" },
          { path: "[].activityGroup", label: "Grupo" },
      ...
      {
        name: "evo_unidade",
        description: "Endereço, telefone e horário de funcionamento da unidade.",
        ...
        root: ["$"],
        fields: [
          { path: "name", label: "Unidade" },
          { path: "address", label: "Endereço" },
          { path: "number", label: "Número" },
          { path: "neighborhood", label: "Bairro" },
          { path: "city", label: "Cidade" },
          { path: "telephone", label: "Telefone" },
          { path: "businessHours[].weekDay", label: "Dia" },
          { path: "businessHours[].hoursFrom", label: "Abre" },
          { path: "businessHours[].hoursTo", label: "Fecha" }
        ],
        maxChars: 1200
      }
